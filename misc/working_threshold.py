...
def _ucr_to_dictionary(fname):
    x_train, y_train = _readucr('/home/ubuntu/Keras_implementation/UCR/' + fname + '/' + fname + '_TRAIN')
    x_test, y_test = _readucr('/home/ubuntu/Keras_implementation/UCR/' + fname + '/' + fname + '_TEST')
...
def _split_data(x_whole, y_whole):
    """
    Split whole data into training and test data with certain ratio from each label
    :param x_whole: A matrix in which rows correspond to feature vectors
    :param y_whole: A vector containing labels
...
flist = ['SonyAIBORobotSurface', 'Lighting7', 'Cricket_X']
...
hist = model.fit(x_train, Y_train, batch_size=batch_size, nb_epoch=NUM_EPOCHS+300, verbose=1,
