...
rlist = ['dataminesjc', 'nbcuni-centralperk2', 'rubicon-fmap', 'gamut-prod', 'rubicon-fbmp', 'nbcuni-superstore', 'jumpshot-jsc', 'rubicon-fbmq', 'rally-health-integration', 'prudential-nj-exp2', 'rubicon-fmaq']
realm = rlist[0]
...
for LEARNING_RATE in LEARNING_RATEs:
    for RHO in RHOs:
        for DECAY in DECAYs:
            ...
            optimizer = keras.optimizers.Adadelta(lr=LEARNING_RATE, decay=DECAY, rho=RHO)
            model.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics=['accuracy'])

            reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.5, patience=200, min_lr=.001)

            early_stopping = EarlyStopping(monitor='loss', min_delta=.001, patience=5, mode='auto')

            model_cp = ModelCheckpoint('FINAL', monitor='val_loss', verbose=0, save_best_only=True, mode='auto', period=1)
            ...
            filename = LEARNING_RATE + '_' + RHO + '_' + DECAY + '_' + realm + '_model'
